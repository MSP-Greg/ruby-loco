# Updates Microsoft/vcpkg packages needed to build Ruby must be run from
# the base Microsoft/vcpkg folder, typically C:/vcpkg
...
if (Test-Path -Path vcpkg.exe -PathType Leaf ) {
  ...
  ./bootstrap-vcpkg.bat
  ./vcpkg upgrade --no-dry-run --triplet=x64-windows $vcpkg_depends.split(' ')
} else {
