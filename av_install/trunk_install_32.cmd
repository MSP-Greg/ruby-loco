...
@set    gdbm=mingw-w64-i686-gdbm-1.10-2-any.pkg.tar.xz
@set openssl=mingw-w64-i686-openssl-1.1.0.f-1-any.pkg.tar.xz
@set    r_fn=ruby_trunk_32.7z
...
@pacman -Rdd --noconfirm mingw-w64-i686-gdbm > nul
...
@pacman -Rdd --noconfirm mingw-w64-i686-openssl > nul
@pacman -Udd --noconfirm --force /c/%openssl%     > nul

@echo --------------------------------------------------------------- MinGW Package Check
@bash -lc "pacman -Qs i686\.\+\(gcc\|gdbm\|openssl\) | sed -n '/^local/p' | sed 's/^local\///' | sed 's/ (.\+$//'"
